...
- `app/` — routes: `/` (landing), `/board` (live kanban), `/principles`, `/skills`, `/contribute`, `/qr`
- `lib/github.ts` — fetches issues and maps labels to board columns (`idea`/`discussing`/`doing`/`done`)
- `lib/config.ts` — site constants (repo, endorse threshold)
...
6. Everything must work without JavaScript-heavy client state — server components by default, client components only for interactivity (see `components/BoardView.tsx`).
