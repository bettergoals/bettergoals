...
function IdeaCard({
  ...
  return (
    <div
      ...
      className={`cursor-grab rounded-xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md active:cursor-grabbing ${
...
export default function BoardView({ initial }: { initial: Board }) {
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-soft">
          {board.source === "github" ? (
            <>Live from GitHub · drag cards between columns · {ENDORSE_THRESHOLD}+ 👍 = endorsed</>
          ) : (
            ...
            <div className="flex min-h-16 flex-col gap-3">
              {board.columns[key].length === 0 ? (
                ...
                board.columns[key].map((idea) => (
                  <IdeaCard key={idea.number} idea={idea} column={key} onMove={moveIdea} />
                ))
              )}
            </div>
          </section>
