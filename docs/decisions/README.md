...
| [0007](0007-the-coaches-are-briefed-on-the-pattern.md) | **The coaches are briefed on the pattern, from the constants the page renders** — one brief, built from `lib/okrPattern.ts`, reaches both coaches, so the framework the site teaches and the framework the coach coaches to cannot drift | Decided, 15 Sep 2026 | Anything that tells a coach what a good OKR looks like |
